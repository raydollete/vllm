...
class DFlashProposer(SpecDecodeBaseProposer):
    ...
    def build_per_group_and_layer_attn_metadata(
        ...
        # SWA draft layers default to a causal sliding-window mask unless
        # dflash_config.causal overrides causality for all layers, matching
        # _resolve_layer_attention in the draft model definition.
        config_causal = self.dflash_config.get("causal", None)
        swa_causal = True if config_causal is None else bool(config_causal)
        ...
        for attn_group in self.draft_attn_groups:
            ...
            # forward context. Keep the group metadata for group-level spec
            # decode checks, and specialize only the SWA layers whose
            # causality differs from the group's.
            causal_layers = sliding_layer_names & set(attn_group.layer_names)
            if causal_layers and swa_causal != self.dflash_causal:
                causal_attn_metadata = (
                    attn_group.get_metadata_builder().build_for_drafting(
                        common_attn_metadata=group_cad.replace(causal=swa_causal),
                        draft_index=draft_index,
                    )
                )
        ...
        for layer_name, attn_metadata in per_layer.items():
            expected_causal = (
                swa_causal if layer_name in sliding_layer_names else self.dflash_causal
            )
            assert getattr(attn_metadata, "causal", None) is expected_causal, (
                f"Attention metadata for layer {layer_name} does not have"
                f" causal={expected_causal} support, which is required for DFlash."
